...
import { speakKorean, initTTS } from "@/src/lib/tts"
...
export function PhraseBubble({ phrase }: PhraseBubbleProps) {
  const handleSpeak = async () => {
    // ユーザー操作内で初期化＋再生
    await initTTS()
    await speakKorean(phrase.ko)
  }

  return (
    <Card
      className="p-4 mb-3 bg-gradient-to-r from-blue-50 to-white border-l-4 border-l-blue-500 cursor-pointer hover:shadow-md transition-shadow"
      onClick={handleSpeak}
    >
